...
function CriderGPTHelper:getCreatorInfo()
    return {
        ...
        description = "CriderGPT Helper was developed by Jessie Crider, FFA Historian, to bring AI and agriculture together inside FS25."
    }
end
...
print("Tier System: Free / Plus / Pro / Lifetime 2 shop-based upgrades.")
print("All systems initialized successfully (Offline Mode).")
print("------------------------------------------------------")
print("[CriderGPT Apollo] Mod registered and ready 2 found by FS25 Mod Manager.")
print(" CriderGPT Helper Mod validated and ready for ModHub submission.")
print("[CriderGPT Apollo] Release build verified 2 ready for ModHub submission.")

print("[CriderGPT Apollo] v1.5.0.0 initialized 2 see modDesc.xml for changelog.")
...
function CriderGPTHelper:onMapLoaded()
    print(" [CriderGPT Apollo v1.5.0.0] by Jessie Crider (FFA Historian) 2 Registered Successfully.")
...
local function checkModDesc()
    ...
    if file then
        ...
        file:close()
        if not string.find(contents, "<descVersion>80</descVersion>") and not string.find(contents, "descVersion=\"80\"") then
            print(" [CriderGPT Apollo] modDesc.xml not found or invalid 2 mod may not appear in game list.")
        end
    else
        print(" [CriderGPT Apollo] modDesc.xml not found or invalid 2 mod may not appear in game list.")
    end
...
local function checkFolderName()
    local folderName = "FS25_CriderGPTHelper"
    local cwd = string.match(debug.getinfo(1, "S").source, "([^/]+)$")
    if cwd ~= folderName then
        print(" [CriderGPT Apollo] Folder name mismatch; rename internal folder to FS25_CriderGPTHelper.")
    end
end
checkFolderName()

print(" [CriderGPT Apollo] Visibility check complete 2 Mod should appear in the FS25 'Select Mods' screen.")
...
local originalStartAIJob = AIVehicleUtil.startJob
    ...
    if vehicle ~= nil and g_currentMission ~= nil then
        ...
        print("2 [CriderGPT Helper] Worker hire event triggered.")
    end
...
local originalStopAIJob = AIVehicleUtil.stopCurrentAIJob
    ...
    if vehicle ~= nil and g_currentMission ~= nil then
            g_currentMission:addExtraPrintText("CriderGPT: Worker dismissed 2 taking a break.")
...
-- ================================================================
-- Some versions of FS25 use "input_AI_START"/"input_AI_STOP" instead of HIRE/DISMISS
...
print(" [CriderGPT Helper] Ready 2 UI override and alerts active.")
...
-- ================================================================
ApolloKnowledgeBase = {
    yield = "Yield depends on soil quality and crop type. Check your field2s precision data.",
...
function CriderGPTHelper:onObjectSold(storeItem)
    if storeItem == "CriderGPT Apollo Core" then
        ...
        print("[CriderGPT Apollo] Plan sold 2 reverted to Free tier.")
        if g_currentMission ~= nil and g_currentMission.addExtraPrintText ~= nil then
...
function CriderGPTApollo:getProductionStatus()
    ...
    for _, prod in pairs(g_currentMission.productionChainManager.productions or {}) do
        ...
        reply = reply .. string.format("%s 2 %d L in, %d L out. ", name, input, output)
    end
...
print("[CriderGPT Apollo] Farm Neural Core Online 2 All Systems Linked (Offline Mode).")


-- ================================================================
-- 2 CriderGPT Chat UI Tab (Groundwork)
-- ================================================================
-- This is a basic structure for a new ESC menu tab using FS25's GUI system
